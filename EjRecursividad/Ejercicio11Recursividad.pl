...
clase(X,Y) :- es_un(X,Y).
clase(X,Y) :- es_un(X,Z),clase(Z,Y).

atributos(X,Atributo,Valor) :- atributo(X,Atributo,Valor).
atributos(X,Atributo,Valor) :- es_un(X,Z), atributos(Z,_,_),atributo(X,Atributo,Valor).